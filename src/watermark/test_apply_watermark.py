...
def expected_original_encoded_bytes():
  with open('../data/expected_encoded/encoded_original.jpeg', 'rb') as f:
    img_bytes = io.BytesIO(f.read()).read()
  return img_bytes
...
class TestColorConversion(unittest.TestCase):
  ...
  def test_encode(self):
    for name, input_data, expected_encoded in [
      ...
      encoded_bytes_jpg, encoded_bytes_png = apply_watermark(
      ...
      watermark_jpg = decode_watermark(
        encoded_bytes_jpg, wm_length=len(WATERMARK) * 8)
      self.assertEqual(WATERMARK, watermark_jpg,
                       f"watermark != expected for {name}")
      watermark_png = decode_watermark(
        encoded_bytes_png, wm_length=len(WATERMARK) * 8)
      self.assertEqual(WATERMARK, watermark_png,
                       f"watermark != expected for {name}")
  ...
  #     img_bytes = input_data[0]
  #     file_type = input_data[1]
  #     encoded_jpg_bytes, encoded_png_bytes = apply_watermark(
  #         img_bytes, file_type, watermark=WATERMARK, resize_for_social_media=True)

  #     encoded_jpg = Image.open(encoded_jpg_bytes)
  #     encoded_png = Image.open(encoded_png_bytes)

  #     width_jpg, height_jpg = encoded_jpg.size
  #     self.assertEqual(new_expected_size, (width_jpg, height_jpg))
  #     width_png, height_png = encoded_png.size
  #     self.assertEqual(new_expected_size, (width_png, height_png))
